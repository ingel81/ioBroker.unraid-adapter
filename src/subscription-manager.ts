...
export class SubscriptionManager {
     ...
     * @returns Promise that resolves when subscription is created
     */
    private subscribeToMemory(): void {
        const subscription = this.apolloClient.subscribe<{
            systemMetricsMemory: { percentTotal?: number; total?: number; used?: number; free?: number };
        }>(`
     ...
     */
    private async handleMemoryUpdate(memory: {
        percentTotal?: number;
        total?: number;
        used?: number;
        free?: number;
     ...
     */
    private shouldUpdate(metricId: string): boolean {
        const now = Date.now();
